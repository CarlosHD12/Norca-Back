...
public class VentaController {
    ...
    public ResponseEntity<VentaResponseDTO> registrarVenta(@Valid @RequestBody VentaRegistroDTO dto) {
        VentaResponseDTO response = ventaService.registrarVenta(dto);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @PatchMapping("/anular/venta/{id}")
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'AYUDANTE')")
    public ResponseEntity<Page<VentaListResponseDTO>> listarVentas(@PageableDefault(size = 20, sort = "idVenta", direction = Sort.Direction.DESC) Pageable pageable) {
