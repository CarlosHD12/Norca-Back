...
@Repository
public interface PrendaRepos extends JpaRepository<Prenda, Long>, JpaSpecificationExecutor<Prenda> {
    ...
    @EntityGraph(attributePaths = {
            ...
            "lotes",
            "lotes.inventarios",
            "lotes.inventarios.talla"
    })
    Optional<Prenda> findDetalleByIdPrenda(Long idPrenda);
    @Query("""
    SELECT p
    FROM Prenda p
    ...
    WHERE p.activo = true
    ORDER BY p.idPrenda DESC
    """)
    Page<Prenda> listarPrendas(Pageable pageable);
    long countByActivoTrue();
    long countByEstado(Prenda.EstadoPrenda estado);
    long countByFechaRegistroAfter(LocalDateTime fecha);
    long countByEstadoAndFechaRegistroAfter(Prenda.EstadoPrenda estado, LocalDateTime fecha);
//
//
