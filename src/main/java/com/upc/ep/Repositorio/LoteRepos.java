...
import com.upc.ep.DTO.LoteRegistroDTO;
import com.upc.ep.DTO.LoteResponseDTO;
...
public interface LoteRepos extends JpaRepository<Lote, Long> {
    ...
    WHERE l.idLote = (
        SELECT l2.idLote
        FROM Lote l2
        WHERE l2.prenda.idPrenda = :idPrenda
        ORDER BY l2.fechaIngreso DESC
        LIMIT 1
    )
    """)
    Optional<Lote> obtenerUltimoLotePrenda(Long idPrenda);
    long countByActivoTrue();
    long countByFechaIngresoAfter(LocalDateTime fecha);
    @Query("""
        SELECT COALESCE(SUM(l.stockActual *(l.precioCompraTotal / l.cantidadInicial)),0)
        FROM Lote l
    ...
    @Query("""
    SELECT COALESCE(
        SUM(l.precioCompraTotal),
        0
    )
    FROM Lote l
    WHERE l.fechaIngreso >= :fecha
    """)
    BigDecimal obtenerInversionUltimoMes(@Param("fecha") LocalDateTime fecha);
