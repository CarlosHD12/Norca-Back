...
public interface VentaRepos extends JpaRepository<Venta, Long> {
    ...
    FROM DetalleVenta d
    WHERE d.inventario.lote.prenda.idPrenda = :idPrenda
      AND d.venta.estadoVenta = com.upc.ep.Entidades.Venta.EstadoVenta.COMPLETADA
      AND d.venta.fechaHora >= :fechaInicio
    """)
    Long obtenerUnidadesVendidas(
            @Param("idPrenda") Long idPrenda,
